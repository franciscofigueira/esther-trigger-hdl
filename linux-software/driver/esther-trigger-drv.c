...
ssize_t _pcie_read(struct file *filp, char *buf, size_t count, loff_t *f_pos) {
  ...
  if ((count > maxByteSize)) { // DMA_MAX_BYTES
    printk(KERN_ALERT "_pcie_read error count %d, dmaByteSize %d\n", (int)count,
           maxByteSize);
    retval = -1;
    goto out;
  }
  ...
  PDEBUGG("_pcie_read command 0x%08x\n", cReg.reg32);
  cReg.cmdFlds.DmaE = 1;
  //
  ...
  */
  ...
  /*Linux gkernel <4.15 rv = copy_to_user(buf, data, count);*/
  if (rv) {
    printk(KERN_ERR "_pcie_read: raw copy_to_user error:%d\n", rv);
    return -EFAULT;
  }
  PDEBUGG("read curr_buf 0x%X \n", pcieDev->curr_buf);
...
int _bridge_mmap(struct file *filp, struct vm_area_struct *vma) {
  ...
  /*xdev = xcdev->xdev;*/
  printk(KERN_INFO "vm_pgoff=%ld, has DMA: %d\n", vma->vm_pgoff,
         0); // off=0, has DMA: 0
...
int _dmach0_open(struct inode *inode, struct file *filp) {
  ...
  // *****************************
  up(&pcieDev->open_sem);
  /*printk(KERN_INFO "Open dmach0_mmap\n");*/
...
int _dmach0_mmap(struct file *filp, struct vm_area_struct *vma) {
  ...
  buffer_bus = pcieDev->dmaIO.buf[0].addr_hw;
  buffer_virt = pcieDev->dmaIO.buf[0].addr_v;
  byteSize = ioread32((void *)&pcieDev->pModDmaHregs->dmaByteSize);
  /** See discussion on  */
  //  set_memory_uc(buffer_bus, (byteSize / PAGE_SIZE));
  /**Track the usage of those pages and use set_memory_wb() before the page is
   * freed to free pool*/
  if (vma->vm_pgoff == 0) {
    //    Using dma_mmap_coherent printk(KERN_INFO "Using dma_mmap_coherent\n");
    rv = dma_mmap_coherent(NULL, vma, buffer_virt, buffer_bus,
                           byteSize * DMA_BUFFS);
  }
  if (rv)
    return -EAGAIN;
...
int setupDMA(PCIE_DEV *pcieDev) {
  ...
  for (i = 0; i < DMA_BUFFS; i++) {
    ...
    iowrite32(buffer_bus + i * byteSize,
              (void *)&pcieDev->pModDmaHregs->dmaBusAddr[i]);
    pcieDev->dmaIO.buf[i].addr_hw = buffer_bus + i * byteSize;
    pcieDev->dmaIO.buf[i].addr_v = buffer_virt + i * byteSize;
  }
  ...
  if (!buffer_virt || !buffer_bus) {
    printk(KERN_ERR
           "setupDMA Poll dma_alloc_coherent error(v:%p hw:%p). EXIT\n",
           buffer_virt, (void *)buffer_bus);
    return -ENOMEM;
  }
...
void _remove(struct pci_dev *pdev) {
  ...
  for (i = 0; i < DMA_BUFFS; i++) {
    // Clear WRITE pci Mem Address registers
